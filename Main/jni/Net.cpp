...
float Net::func(float x){
	return 1.0/(1.0+exp(-x));
}
float Net::dfunc(float x){
	return x*(1.0-x);
}
void Net::forward_err(float h){
...
Net::Net(int a, int b, int c, int d){
	int k=0;
	std::vector<int> temp;
	for(int j=0;j<a;j++){
		temp.push_back(k);
...
std::vector<float> Net::forward(std::vector<float> a){
...
void Net::backward(float a, float h){
	for(int i=0;i<net[net.size()-1].size();i++){
